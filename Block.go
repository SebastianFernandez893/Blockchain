...
func blockToString(b *Block) {
	...
	fmt.Println("\ttransaction:", b.tx[0])
...
func setBlockHash(b *Block) {
	...
	hashIn := strconv.Itoa(b.confirmations) + strconv.Itoa(b.size) + strconv.Itoa(b.strippedSize) +
		strconv.Itoa(b.weight) + strconv.Itoa(b.version) + b.versionHex + b.merkleRoot +
		b.tx[0] + strconv.FormatInt(b.timestamp, 10) + strconv.FormatInt(b.medianTimestamp, 10) + strconv.Itoa(b.nonce) +
		b.bits + strconv.Itoa(b.difficulty) + b.chainWork + b.prevBlockHash
...
/*
	creates a new block given the previous block and new nonce
	generates fake data for non-utilized data fields
	@output new block
*/
func createBlock(nonce int, difficulty int, prevBlock *Block) Block {
